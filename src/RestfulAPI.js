      ...
      try {
        ...
        console.log(
            `fetch 전송값 : ${response}
             ...
             json실행결과문자화 : ${JSON.stringify(result, null, 2)}`
            )
        ...
        setLoading(false); // 반드시 1회 , 비동기 데이터 , 데이터 아직 저장중이라 기다려야할때 로딩화면
      }
    };
  ...
  return (
    <div className="App">
      ...
      <ul style={{"display" : "flex" , "flexWrap" : "wrap"}}>
        {data.map((item, index) => (
            <li key={index}>{item.someField}
                <img src ={`http://${item.CON_IMGFILENAME}`} style={imgstyle}></img>
            </li>
        ))}
      </ul>
    </div>
